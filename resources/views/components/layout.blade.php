...
<head>
    ...
    <!-- Favicon -->
    <link type="image/png" href="/public/favicon.png" rel="icon">
...
<body class="flex min-h-screen flex-col overflow-y-auto">
<style>
    html,
    body {
...
@if (session('success') || session('error'))
    <div
        class="{{ session('success') ? 'bg-green-50/95 border border-green-200 text-green-800' : 'bg-red-50/95 border border-red-200 text-red-800' }} fixed right-4 top-20 z-50 flex items-center gap-3 rounded-lg px-4 py-3 shadow-lg backdrop-blur-sm transition-all duration-500 ease-in-out"
        id="alert-notification" style="opacity: 0; transform: translateX(100%);">
        ...
        {{-- Close Button --}}
        <button
            class="{{ session('success') ? 'text-green-600 focus:ring-green-500' : 'text-red-600 focus:ring-red-500' }} ml-2 shrink-0 rounded-md p-1 transition-colors hover:bg-white/50 focus:outline-none focus:ring-2 focus:ring-offset-2"
...
@endif
<nav class="fixed top-0 z-50 w-full border-b border-slate-100 bg-white/90 backdrop-blur-sm">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 items-center justify-between">
            <div class="flex shrink-0 cursor-pointer items-center gap-2">
                <div class="rounded-lg bg-indigo-600 p-1.5 text-white">
                ...
                <a href="{{ route('home') }}"> <span
                        class="text-xl font-bold tracking-tight text-slate-900">Chirper</span></a>
            </div>
            <div class="flex items-center gap-4">
                @auth
                    ...
                    <form action="{{route('logout')}}" method="post">
                        @csrf
                        <button type="submit"
                                class="rounded-full bg-indigo-600 px-5 py-2.5 text-sm font-medium text-white shadow-lg shadow-indigo-500/30 transition hover:bg-indigo-700">Logout</button>
                    </form>
                @else
                    <a class="text-sm font-medium text-slate-600 transition hover:text-indigo-600"
                       href="{{ route('login.index') }}">
                        Log in
                    </a>
                    <a class="rounded-full bg-indigo-600 px-5 py-2.5 text-sm font-medium text-white shadow-lg shadow-indigo-500/30 transition hover:bg-indigo-700"
...
<main class="mx-auto w-full max-w-7xl flex-1 px-4 py-24 sm:px-6 lg:px-8">{{ $slot }} </main>

<footer
    class="bottom-0 left-0 right-0 flex flex-row items-center justify-center border-t border-slate-100 bg-white/90 py-4 backdrop-blur-sm">
    <p class="flex items-center gap-2 text-sm font-medium text-slate-600">
        <span class="text-lg">🦺</span>
        <span>© 2025 <span class="font-semibold text-slate-700">Laravel Parade</span></span>
    </p>
</footer>
</body>
